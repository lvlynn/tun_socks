 ...
 *    %[0][width|m][u][x|X]i    nt_int_t/nt_uint_t
 *    %[0][width][u][x|X]D      int32_t/uint32_t
 *    %[0][width][u][x|X]L      int64_t/uint64_t
 *    %[0][width|m][u][x|X]A    nt_atomic_int_t/nt_atomic_uint_t
...
nt_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
    ...
    uint64_t               ui64, frac;
    nt_msec_t             ms;
    nt_uint_t             width, sign, hex, max_width, frac_width, scale, n;
    nt_str_t             *v;
...
/*
 * We use nt_strcasecmp()/nt_strncasecmp() for 7-bit ASCII strings only,
 * and implement our own nt_strcasecmp()/nt_strncasecmp()
 * to avoid libc locale overhead.  Besides, we use the nt_uint_t's
...
/*
 * nt_strstrn() and nt_strcasestrn() are intended to search for static
...
/*
 * nt_strlcasestrn() is intended to search for static substring
...
/*
 * nt_utf8_decode() decodes two and more bytes UTF sequences only
 * the return values:
